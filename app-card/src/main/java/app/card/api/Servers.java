		...
		builder.addInterceptor(new Interceptor()
		{
			...
			public Response intercept(Chain chain) throws IOException{
				...
				if(req.url().toString().startsWith(Server.BASE_URL)){
					...
					String token=SharedPreferencesUtils.getString(App.get(),"token","");
					...
					if(!TextUtils.isEmpty(token))
						builder.addHeader("KM-Token", token);
					builder.addHeader("KM-Device-Type", Server.DEVICE_TYPE);
					
					//builder.addHeader("p-token", token);
